...
*/
    ...
    fun test1(case: TC) {
        ...
        println("$coeffA (${coeffA.digitCount}) / 10**$pow10 = expected:$expected expectedInexact:${case.expectedInexact}")
        coeffObserved.scalePow10(coeffA, -pow10, case.sign, ctx)
        val observed = coeffObserved.toBigInteger()
        if (! observed.equals(expected))
            println("$coeffA (${coeffA.digitCount}) / 10**$pow10 = $coeffObserved (${coeffObserved.digitCount})  expected:$expected expectedInexact:${case.expectedInexact}")
        if (case.expectedInexact != ctx.inexact)
            println("INEXACT mismatch $coeffA (${coeffA.digitCount}) * 10**$pow10 = $coeffObserved (${coeffObserved.digitCount})  inexact expected:${case.expectedInexact} observed:${ctx.inexact}")
