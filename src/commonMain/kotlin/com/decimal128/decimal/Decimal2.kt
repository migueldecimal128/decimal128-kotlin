...
class Decimal2 private constructor(
    // pronounced:
    ...
    companion object {
        ...
        internal operator fun invoke(sign: Boolean, qExp: Int,
                                     dw1: Long, dw0: Long,
                                     allowNonCanonical: Boolean = false): Decimal2 {
            val bitLen = calcBitLen128(dw1, dw0)
            val digitLen = calcDigitLen128(bitLen, dw1, dw0)
            return Decimal2(sign, qExp, digitLen, bitLen, dw1, dw0, allowNonCanonical)
        }
        ...
        internal operator fun invoke(sign: Boolean, qExp: Int,
            ...
            return Decimal2(Seal(sign, qExp, digitLen, bitLen), dw1.toULong(), dw0.toULong())
        }
        ...
        internal operator fun invoke(sign: Boolean, qExp: Int,
            ...
            verify { digitLen <= 38 }
            verify { digitLen <= 34 || allowNonCanonical }
            return Decimal2(Seal(sign, qExp, digitLen, bitLen), dw1, dw0)
        }


        val POS_ZEROe0 = Decimal2(Seal(false, 0, 0, 0), 0uL, 0uL)
        val NEG_ZEROe0 = POS_ZEROe0.negate()
        val POS_ONEe0 = from(1)
        val NEG_ONEe0 = POS_ONEe0.negate()
        val POS_INFINITY = Decimal2(false, NON_FINITE_INF, 0, 0, 0uL, 0uL)
        val NEG_INFINITY = POS_INFINITY.negate()
        val POS_QNAN = Decimal2(false, NON_FINITE_QNAN, 0, 0, 0uL, 0uL)
        val NEG_QNAN = POS_QNAN.negate()
        val POS_SNAN = Decimal2(false, NON_FINITE_SNAN, 0, 0, 0uL, 0uL)
        ...
        fun from(n: Int): Decimal2 = from(n.toLong())

        fun from(w: UInt): Decimal2 = from(w.toULong())

        fun from(l: Long): Decimal2 {
            val mask = l shr 63
            val abs = ((l xor mask) - mask).toULong()
            val signBit = (mask.toInt() ushr 31)
            val seal = Seal(signBit, 0, 0uL, abs)
            return Decimal2(seal, 0uL, abs)
        }

        fun from(dw: ULong): Decimal2 = Decimal2(Seal(false, 0, 0uL, dw), 0uL, dw)
         ...
         */
        fun from(str: String): Decimal2 = DecParsePrint.parseDecimal(str)

        fun zero() = POS_ZEROe0

        fun zero(sign: Boolean = false, qExp: Int): Decimal2 {
            if (qExp == 0)
            ...
            val zero = Decimal2(seal, 0uL, 0uL)
            return zero
        }
        ...
        fun infinityNonCanonical(sign: Boolean, dw1: ULong, dw0: ULong): Decimal2 =
            if ((dw1 or dw0) == 0uL)
                infinity(sign)
            else
                Decimal2(sign, NON_FINITE_INF, dw1, dw0, allowNonCanonical = true)

        fun NaN() = POS_QNAN

        fun NaN(sign: Boolean, signaling: Boolean = false): Decimal2 {
            return when {
        ...
        fun NaN(sign: Boolean = false, signaling: Boolean = false, payloadDw0: ULong = 0uL): Decimal2 =
            NaN(sign, signaling, 0L, payloadDw0.toLong())

        fun NaN(sign: Boolean = false, signaling: Boolean = false, payloadDw0: Long = 0L): Decimal2 =
        ...
        fun NaN(
            ...
            return Decimal2(sign, qExp, digitLen, bitLen, p1, p0, allowOversizePayload)
        }

        internal inline fun bothFnz(x: Decimal2, y: Decimal2): Boolean {
        ...
        internal fun hasNaN(x: Decimal2, y: Decimal2): Boolean =
            x.qExp >= NON_FINITE_QNAN || y.qExp >= NON_FINITE_QNAN

        internal fun neitherIsNaN(x: Decimal2, y: Decimal2) =
            x.qExp < NON_FINITE_QNAN && y.qExp < NON_FINITE_QNAN
    ...
    fun compareTotalOrderTo(other: Decimal2) = DecCompare.cmpTotalOrder(this, other)

    fun isTotalOrder(other: Decimal2) = compareTotalOrderTo(other) <= 0
     ...
     */
    fun compareTotalOrderMagTo(other: Decimal2) = DecCompare.cmpTotalOrderMag(this, other)

    fun isTotalOrderMag(other: Decimal2) = compareTotalOrderMagTo(other) <= 0
     ...
     */
    fun compareJavaStyleTo(other: Decimal2): Int = DecCompare.cmpJavaStyle(this, other)

    fun equalsJavaStyle(other: Decimal2): Boolean = DecCompare.eqJavaStyle(this, other)
     ...
     */
    override fun compareTo(other: Decimal2): Int = compareJavaStyleTo(other)
    ...
    override fun equals(other: Any?): Boolean =
        other is Decimal2 && equalsJavaStyle(other)
     ...
     * ```
     */
    infix fun bitwiseEQ(other: Decimal2): Boolean =
    ...
    fun negate(): Decimal2 = Decimal2(seal.negate(), dw1, dw0)

    fun abs(): Decimal2 = if (isNegative()) negate() else this

    // this is only provided for IEEE754-2019 completeness.
    // in an immutable world it serves no purpose
    fun copy(): Decimal2 = Decimal2(seal, dw1, dw0)

    fun copySign(other: Decimal2) =
        if (isNegative() xor other.isNegative()) negate() else this

    fun copyAbs(other: Decimal2) = Decimal2(seal.abs(), dw1, dw0)
     ...
     */
    fun sameQuantum(other: Decimal2) =
