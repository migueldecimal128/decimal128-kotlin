...
object Magia {
    ...
    /**
     * Converts a non-negative binary value from a byte array into a [Magia] limb array.
     *
     * The input represents an unsigned integer in either big-endian or little-endian
     * byte order. Leading zero bytes are ignored. The resulting [IntArray] contains
     * 32-bit limbs in least-significant-first order, suitable for internal arithmetic
     * operations within [Magia].
     *
     * This function does **not** interpret any sign bit; the input is assumed to
     * represent a non-negative value.
     *
     * @param isBigEndian  `true` if the input bytes are in big-endian order, or `false`
     *                     for little-endian order.
     * @param bytes        Source array containing the bytes to decode.
     * @param off          Starting offset within [bytes].
     * @param len          Number of bytes to read from [bytes].
     *
     * @return An [IntArray] containing the decoded 32-bit limbs, least-significant limb first.
     *         Returns [ZERO] if all bytes in the specified range are zero.
     *
     * @throws IllegalArgumentException if [off] or [len] are out of range.
     *
     * @see Magia
     */
    internal fun fromNonNegativeBytes(isBigEndian: Boolean, bytes: ByteArray, off: Int, len: Int):
            IntArray {
        if (off < 0 || len < 0 || len > bytes.size - off)
            throw IllegalArgumentException()
        if (len == 0)
        ...
        val ibLsb = if (isBigEndian) ibLast else off
        var ibMsb = if (isBigEndian) off else ibLast
        ...
        if (remaining > 0) {
            // b3 is always 0
            val b2 = if (remaining == 3) bytes[ib + offB2].toInt() and 0xFF else 0
            val b1 = if (remaining >= 2) bytes[ib + offB1].toInt() and 0xFF else 0
            val b0 = bytes[ib].toInt() and 0xFF
