...
class TestJnaBidDpdShim {
    ...
    @Test
    fun test1() {
        val api = JnaBidDpdShim.INSTANCE
        val values = listOf(
            "0", "-0", "123.45", "-9999999999999",
        ...
        for (v in values) {
            ...
            check(api.d128_dpd_from_string(v, d128_dpd) == 0)
            check(api.d128_bid_from_string(v, d128_bid) == 0)

            val sBuf = ByteArray(128)
            api.d128_dpd_to_string(d128_dpd, sBuf, sBuf.size)
            val d128_dpd_rt = sBuf.cString()
            api.d128_bid_to_string(d128_bid, sBuf, sBuf.size)
            ...
            check(api.d64_dpd_from_string(v, d64_dpd) == 0)
            check(api.d64_bid_from_string(v, d64_bid) == 0)

            val sBuf2 = ByteArray(128)
            api.d64_dpd_to_string(d64_dpd, sBuf2, sBuf2.size)
            val d64_dpd_rt = sBuf2.cString()
            api.d64_bid_to_string(d64_bid, sBuf2, sBuf2.size)
