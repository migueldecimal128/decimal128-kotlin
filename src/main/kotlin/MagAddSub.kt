...
object MagAddSub {
    ...
    fun magSub_subtraction_fails_rounding(z: Mag, x: Mag, y: Mag): Residue {
        assert(x.magCompareTo(y) >= 0)
        if (x.qExp == y.qExp) {
            z.qExp = x.qExp
            CoeffSub.coeffSubUnscaled(z.c, x.c, y.c)
        ...
        val qDelta = Math.abs(x.qExp - y.qExp)
        val headroomWithGuard = 1 + PRECISION_34 - x.c.digitLen
        val shiftLeft = Math.min(qDelta, headroomWithGuard)
        val qAlign = x.qExp - shiftLeft
        if (x.qExp > y.qExp) {
            when {
                (x.c.bitLen > 0 && y.c.bitLen > 0) -> {
