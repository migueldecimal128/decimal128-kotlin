...
internal fun mutDecMulImpl(z: MutDec, x: MutDec, y: MutDec, ctx: DecContext): MutDec {
    ...
    if (binopSignature == FNZ_FNZ) {
        ...
        INF_ZER,
        ZER_INF -> ctx.setNanSignalInvalidOperation(z, InvalidCause.MUL_ZERO_BY_INFINITY)
        INF_FNZ,
        FNZ_INF,
    ...
    when (stealTyp(x.steal)) {
        STEAL_TYP_FNZ -> {
            if (! xSign) {
                ...
                ctx.setNanSignalInvalidOperation(z, InvalidCause.SQUARE_ROOT_OF_NEG_FINITE_NON_ZERO)
            }
        }
        STEAL_TYP_ZER -> {
        ...
        STEAL_TYP_INF -> {
            if (! xSign) {
                ...
                ctx.setNanSignalInvalidOperation(z, InvalidCause.SQUARE_ROOT_OF_NEG_INFINITY)
            }
        }
        else -> z.setNanOperandFound(x, ctx)
    }
