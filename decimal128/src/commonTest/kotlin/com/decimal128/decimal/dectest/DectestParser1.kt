...
object DectestParser1 {
    ...
    fun parse(input: String, opName: String = "", formatStyleEngineering: Boolean = false): List<DectestCase1> =
        parseLines(input.lineSequence(), opName, formatStyleEngineering)

    fun parse(lines: Array<String>, opName: String = "", formatStyleEngineering: Boolean = false): List<DectestCase1> =
        parseLines(lines.asSequence(), opName, formatStyleEngineering)

    fun parseLines(lines: Sequence<String>, opName: String = "", formatStyleEngineering: Boolean = false): List<DectestCase1> {
        var env = DectestEnv()
        ...
        for (line in lines) {
            ...
            if (env.isValid()) {
                // Test case
                if ("->" in raw) {
                    val case = DectestCase1.parseDectestCase(raw, env, formatStyleEngineering)
